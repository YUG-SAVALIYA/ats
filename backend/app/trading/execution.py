...
from app.data.models import AtsOrder, Trade, TradeEvent, AtsTradeState, OrderPurpose, Company, OrderAttempt
from app.broker.dhan_client import get_account_context, is_error_response
from app.trading.risk import PreTradeSafetyValidator
from app.trading.levels import compute_initial_levels
from app.trading.state_machine import validate_state_transition

logger = logging.getLogger("ats.execution")
...
def _resolve_account_context(account_id: str):
    import sys
    for mod_name in ("app.trading.execution", "app.broker.dhan_client"):
...
def _log_event(db, trade_id: str, event_type: str, detail: str = "",
               ...
               dhan_account_id: str | None = None) -> None:
    try:
        ...
        if not dhan_account_id:
            logger.warning(f"[EXEC] Cannot log TradeEvent without dhan_account_id for trade {trade_id}")
            return
        db.add(TradeEvent(
...
async def place_market_sell(
    ...
    trade_strategy_type = "SUPERTREND"
    ...
    try:
        ...
        is_safe, safety_reason = PreTradeSafetyValidator.validate_pre_exit(trade, qty)
        if not is_safe:
            raise RuntimeError(f"Exit safety check failed: {safety_reason}")

        trade_account_id = trade.dhan_account_id
        trade_strategy_type = trade.strategy_type or "SUPERTREND"
        ...
        order_rec = AtsOrder(
            id=order_rec_id,
            dhan_account_id=trade.dhan_account_id,
            trade_id=trade_id,
            dhan_order_id=None,
        ...
        _log_event(db1, trade_id, "EXIT_CLAIMED",
                   ...
                   quantity=qty, dhan_account_id=trade_account_id)
    ...
    # ═════════════════════════════════════════════════════════════════════════
    client = _resolve_account_context(trade_account_id)
    if not trade_account_id or trade_account_id != client.dhan_account_id:
        logger.critical(f"[SAFETY_ABORT] Trade {trade_id} account {trade_account_id} != Client account {client.dhan_account_id}")
        return AtsOrder(id=order_rec_id, status="FAILED")
    ...
    try:
        ...
        try:
            ...
            db_err.commit()
            _log_event(db_err, trade_id, "EXIT_ORDER_UNKNOWN", f"Network timeout/error during SELL: {net_err}", quantity=qty, dhan_account_id=trade_account_id)
            if ord_item:
        ...
        ret_ord = AtsOrder(id=order_rec_id, dhan_account_id=trade_account_id, trade_id=trade_id, correlation_id=correlation_id, order_purpose=purpose_str, transaction_type="SELL", security_id=str(security_id), quantity=int(qty), order_type="MARKET", product_type=product_type, exchange_segment="NSE_EQ", status="UNKNOWN")
        return ret_ord
    ...
    try:
        ...
        if is_error_response(res) or str(res.get("status", "")).lower() in ("failure", "error"):
            http_code = res.get("http_code", 400)
            remarks = res.get("remarks") or res
            err_msg = str(remarks.get("error_message") if isinstance(remarks, dict) else remarks)
            
            if http_code == 503 or "Network Error" in err_msg:
                logger.warning(f"[EXEC] Network timeout on SELL. Marking as UNKNOWN: trade={trade_id} err={err_msg}")
                if ord_item:
                    ord_item.status = "UNKNOWN"
                ...
                db_resp.commit()
                _log_event(db_resp, trade_id, "EXIT_UNKNOWN", f"Network timeout: {err_msg}", quantity=qty)
                if ord_item:
                    db_resp.expunge(ord_item)
                    return ord_item
                return AtsOrder(id=order_rec_id, status="UNKNOWN")
            ...
            if ord_item:
                db_resp.expunge(ord_item)
                return ord_item
            ret_ord = AtsOrder(id=order_rec_id, dhan_account_id=trade_account_id, trade_id=trade_id, correlation_id=correlation_id, order_purpose=purpose_str, transaction_type="SELL", security_id=str(security_id), quantity=int(qty), order_type="MARKET", product_type=product_type, exchange_segment="NSE_EQ", status="REJECTED")
            return ret_ord
        ...
        db_resp.commit()
        _log_event(db_resp, trade_id, "EXIT_ORDER_ACCEPTED", f"MARKET SELL accepted by Dhan: dhan_order_id={dhan_order_id}", quantity=qty, dhan_account_id=trade_account_id)
        ...
        ret_ord = AtsOrder(id=order_rec_id, dhan_account_id=trade_account_id, trade_id=trade_id, strategy_type=trade_strategy_type, dhan_order_id=dhan_order_id, correlation_id=correlation_id, order_purpose=purpose_str, transaction_type="SELL", security_id=str(security_id), quantity=int(qty), order_type="MARKET", product_type=product_type, exchange_segment="NSE_EQ", status="TRANSIT")
        return ret_ord
...
class OrderExecutor:
    ...
    def place_entry_order(
        ...
        *,
        ...
        db = SessionLocal()
        try:
            is_safe, safety_reason = PreTradeSafetyValidator.validate_pre_entry(
                db=db,
                dhan_account_id=dhan_account_id,
            ...
            if not is_safe:
                logger.warning(f"[SAFETY_REJECT] Entry blocked for {trading_symbol}: {safety_reason}")
                return {"status": "blocked", "message": safety_reason, "remarks": safety_reason}
            ...
            client = _resolve_account_context(dhan_account_id)
            if client.dhan_account_id != dhan_account_id:
                raise RuntimeError(f"[SAFETY_ABORT] Account mismatch: trade {dhan_account_id} != client {client.dhan_account_id}")
            ...
            if is_error_response(res) or str(res.get("status", "")).lower() in ("failure", "error"):
                ...
                if http_code == 503 or "Network Error" in err_msg:
                    logger.warning(f"[EXEC] Network timeout on entry. Marking as UNKNOWN: trade={trade_id} err={err_msg}")
                    order_rec.status = "UNKNOWN"
                    order_rec.last_error = err_msg
                    order_rec.updated_at = datetime.now(timezone.utc)
                    trade.ats_state = AtsTradeState.ENTRY_PENDING
                    attempt.status = "UNKNOWN"
                    attempt.error_message = err_msg
                    db.commit()
                    _log_event(db, trade_id, "ENTRY_UNKNOWN", f"Network timeout: {err_msg}", quantity=quantity)
                    return {"status": "unknown", "trade_id": trade_id, "error": err_msg}
            ...
            logger.warning(f"[EXEC] Idempotent constraint hit (duplicate entry attempt blocked) for sec={security_id} account={dhan_account_id}. Error: {e}")
            try:
                db.rollback()
            except Exception:
                pass
            return {"status": "duplicate", "error": "Duplicate entry attempt gracefully aborted."}
        except Exception as exc:
    ...
    def cancel_order(self, dhan_order_id: str, dhan_account_id: str = None) -> dict:
        db = SessionLocal()
        try:
            if not dhan_account_id:
                ats_ord = db.query(AtsOrder).filter(AtsOrder.dhan_order_id == dhan_order_id).first()
                if not ats_ord:
                    return {"success": False, "error": "Order not found in DB to resolve account"}
                trade = db.query(Trade).filter(Trade.id == ats_ord.trade_id).first()
                if not trade:
                    return {"success": False, "error": "Trade not found"}
                dhan_account_id = trade.dhan_account_id
                
            client = get_account_context(dhan_account_id)
            url = f"https://api.dhan.co/v2/orders/{dhan_order_id}"
            res = client.execute_v2_delete(url)
...
def confirm_entry_fill(trade_id: str, fill_price: float, fill_qty: int) -> Optional[Trade]:
    """Top-level helper to confirm entry fill via the singleton OrderExecutor."""
    return get_order_executor().confirm_entry_fill(trade_id, fill_price, fill_qty)
