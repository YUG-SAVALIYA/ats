...
from app.data.models import Trade, AtsTradeState, TradeEvent
...
def _log_event(
    ...
    try:
        ...
        if not dhan_account_id:
            logger.warning(f"[ENGINE] Cannot log TradeEvent without dhan_account_id for trade {trade_id}")
            return
        db.add(TradeEvent(
...
class TradeEngine:
                ...
                if dead_feeds:
                    ...
                    if self.ws_manager:
                        ...
                        for sec_id in dead_feeds:
                            self._last_tick_map[sec_id] = time.time()
    ...
    async def _execute_final_exit(self, trade: Trade, ltp: float, reason: str) -> None:
        ...
        try:
            ...
            if state_str in ("CLOSED", "EXIT_REQUESTED", "EXIT_UNKNOWN", AtsTradeState.CLOSED, AtsTradeState.EXIT_REQUESTED, AtsTradeState.EXIT_UNKNOWN):
                logger.debug(f"[ENGINE] Trade {tid} already in state {state_str} in DB — skip final exit")
                return
            ...
            if t2:
                ...
                if state_val in ("CLOSED", "FAILED", "CANCELLED", "EXIT_REQUESTED"):
    ...
    async def recalculate_active_trades(self) -> int:
        """Retroactively apply current settings to all active trades."""
        db = SessionLocal()
        count = 0
        try:
            active_trades = (
