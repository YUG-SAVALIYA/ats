...
app.data.candles
================
Daily and historical candle synchronization and database storage.
"""
...
from app.data.models import Company, DailyCandle, WeeklyCandle
from app.data.weekly import aggregate_weekly_candles

logger = logging.getLogger("ats.candles")
...
def sync_actionable_companies(delay_sec: float = SYMBOL_FETCH_DELAY_SEC) -> Dict[str, Any]:
    ...
    from app.data.models import Signal, Trade
    db = SessionLocal()
    try:
...
def get_monthly_candles_from_db(company_id: str, limit: int = 30) -> List[Dict]:
    """Read daily candles and aggregate into monthly candles."""
    db = SessionLocal()
    try:
        rows = (
            db.query(DailyCandle)
