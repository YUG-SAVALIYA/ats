"""
database/models.py — ATS SQLAlchemy ORM Models & Enums
======================================================
Database entities for trades, orders, market data, credentials, settings, and events.
"""
...
class Company(Base):
    ...
    id = Column(String(36), primary_key=True, default=generate_uuid)
    dhan_security_id = Column(String(64), index=True, nullable=True)
...
class Signal(Base):
    ...
    expiry_reason = Column(String(255), nullable=True)
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow)
...
class WeeklyCandle(Base):
    ...
    status = Column(String(16), default="completed", nullable=True)  # 'completed' or 'incompleted'
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow)
    updated_at = Column(DateTime(timezone=True), default=datetime.utcnow, onupdate=datetime.utcnow)
...
# ═══════════════════════════════════════════════════════════════════════════════
# ATS EXECUTION ENGINE MODELS
# ═══════════════════════════════════════════════════════════════════════════════

class AtsOrder(Base):
    """Tracks Dhan orders placed by the ATS execution engine."""
...
class OrderAttempt(Base):
    """Audit log of order placement attempts sent to Dhan API."""
