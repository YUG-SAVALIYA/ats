"""
dhan/client.py — Dhan HQ API v2 Singleton Facade & Client
=========================================================
Provides HTTP request wrappers (GET, POST, DELETE) with automatic token refreshment,
error response parsing, and singletons for TradeAccount (execution) and DataAccount (market data).
"""
...
from dhan.auth import DhanAuthManager
from dhan.endpoints import MARKET_FEED_OHLC_URL, MARKET_FEED_LTP_URL, MARKET_FEED_QUOTE_URL

logger = logging.getLogger("ats.dhan.client")
...
class DhanClient:
    ...
    def execute_v2_get(self, endpoint_url: str) -> Any:
        """Executes GET request against Dhan V2 API with auto 401/403 token refresh & empty response handling."""
        ...
        if resp.status_code in (401, 403) or is_auth_error(resp.json() if _is_json(resp) else None):
            logger.warning(f"[CLIENT][{self.account_label}] Auth error from {endpoint_url}. Refreshing token...")
    ...
    def get_marketfeed_ohlc(self, nse_eq_sec_ids: List[int]) -> Dict[str, Any]:
        ...
        logger.info(f"[CLIENT API CALL][{self.account_label}] POST {MARKET_FEED_OHLC_URL} (IDs: {nse_eq_sec_ids})")
        token = self.auth_manager.get_valid_token()
        headers = {
            "Accept": "application/json",
            "Content-Type": "application/json",
    ...
    def get_marketfeed_ltp(self, nse_eq_sec_ids: List[int]) -> Dict[str, Any]:
        ...
        logger.info(f"[CLIENT API CALL][{self.account_label}] POST {MARKET_FEED_LTP_URL} (IDs: {nse_eq_sec_ids})")
        token = self.auth_manager.get_valid_token()
        headers = {
            "Accept": "application/json",
            "Content-Type": "application/json",
    ...
    def get_marketfeed_quote(self, nse_eq_sec_ids: List[int]) -> Dict[str, Any]:
        """Fetch live full market depth quote (5-depth bid/ask & volume) for a list of NSE_EQ security IDs."""
        if not nse_eq_sec_ids:
            return {}

        logger.info(f"[CLIENT API CALL][{self.account_label}] POST {MARKET_FEED_QUOTE_URL} (IDs: {nse_eq_sec_ids})")
        token = self.auth_manager.get_valid_token()
        headers = {
            "Accept": "application/json",
            "Content-Type": "application/json",
        ...
        url = MARKET_FEED_QUOTE_URL
        try:
            resp = requests.post(url, json=payload, headers=headers, timeout=10)
            if resp.status_code in (401, 403):
                logger.warning(f"[CLIENT][{self.account_label}] Marketfeed Quote 401/403 auth error. Refreshing token...")
                if self.auth_manager.refresh_token():
                    headers["access-token"] = self.auth_manager.get_valid_token() or ""
                    resp = requests.post(url, json=payload, headers=headers, timeout=10)
            ...
            if resp.status_code == 200:
                ...
                if isinstance(data, dict) and "data" in data:
                    return data.get("data", {}).get("NSE_EQ", {})
                return {}
            logger.warning(f"[CLIENT][{self.account_label}] Marketfeed Quote API HTTP {resp.status_code}: {resp.text[:150]}")
            return {}
        except Exception as exc:
            logger.error(f"[CLIENT][{self.account_label}] Unexpected error in get_marketfeed_quote: {exc}", exc_info=True)
            return {}
...
def get_dhan_client() -> DhanClient:
    """Returns singleton DhanClient for Trade / Order Execution Account."""
...
def get_dhan_data_client() -> DhanClient:
    """Returns singleton DhanClient for Market Data (falls back to primary account if single account)."""
    ...
    with _lock:
        if _data_client_instance is not None:
            return _data_client_instance
        cfg = load_config()
        cid = cfg.data_client_id or cfg.client_id
        pin = cfg.data_pin or cfg.pin
        totp = cfg.data_totp_secret or cfg.totp_secret
        tok = cfg.data_access_token or cfg.access_token
        ...
        _data_client_instance = DhanClient(
            ...
            totp_secret=totp,
            access_token=tok,
            account_label="DataAccount" if cfg.data_client_id else "TradeAccount"
        )
        return _data_client_instance


def reset_dhan_clients():
    """Resets cached DhanClient singletons so fresh tokens are immediately loaded from DB."""
    global _trade_client_instance, _data_client_instance
    with _lock:
        _trade_client_instance = None
        _data_client_instance = None
    logger.info("[CLIENT] Dhan client singletons reset. Next call will load fresh credentials.")
