"""
trading/orders.py — Order Execution Orchestrator & Two-Phase Exit Claim
======================================================================
Production-grade MARKET BUY and MARKET SELL order placement enforcing atomic
Two-Phase Exit-Claim protocol, idempotency, and Dhan-as-Source-of-Truth architecture.
"""
...
from database.models import (
    AtsOrder, Trade, TradeEvent, AtsTradeState,
    OrderPurpose, Company, OrderAttempt
)
from database.repositories import log_trade_event
from dhan.client import get_dhan_client, is_error_response
from dhan.orders import cancel_dhan_order
from dhan.endpoints import ORDERS_URL
from trading.risk import compute_initial_levels
from trading.trades import validate_state_transition

logger = logging.getLogger("ats.trading.orders")
...
async def place_market_sell(
    ...
    Phase 2 (External HTTP Call):
      ...
      - Dhan Rejected: Updates AtsOrder.status = "REJECTED", trade.ats_state = EXIT_FAILED.
      - Network Timeout: Updates AtsOrder.status = "UNKNOWN", trade.ats_state = EXIT_UNKNOWN.
    """
    if qty <= 0:
        raise ValueError(f"[ORDERS] Cannot place SELL for qty={qty} on trade {trade_id}")
    ...
    try:
        ...
        if trade_state_str == "CLOSED" or active_exit_order:
            ...
            logger.warning(f"[ORDERS] Duplicate SELL blocked for trade {trade_id}: active order/state={status_desc}")
        ...
        log_trade_event(
            db1, trade_id, "EXIT_CLAIMED",
            f"Phase 1 Exit-Claim committed: qty={qty}, purpose={purpose_str}, correlation_id={correlation_id}",
            quantity=qty
        )
        logger.info(f"[ORDERS] Phase 1 Exit-Claim committed for trade {trade_id} (tag={correlation_id})")

    except Exception as exc:
        db1.rollback()
        logger.error(f"[ORDERS] Phase 1 Exit-Claim failed for trade {trade_id}: {exc}")
        raise exc
    finally:
        db1.close()
    ...
    try:
        ...
        logger.error(f"[ORDERS] Network error sending SELL for trade={trade_id}: {net_err}")
        db_err = SessionLocal()
        try:
            ...
            db_err.commit()
            log_trade_event(db_err, trade_id, "EXIT_ORDER_UNKNOWN", f"Network timeout/error during SELL: {net_err}", quantity=qty)
            if ord_item:
        ...
        return AtsOrder(
            id=order_rec_id, trade_id=trade_id, correlation_id=correlation_id,
            order_purpose=purpose_str, transaction_type="SELL", security_id=str(security_id),
            quantity=int(qty), order_type="MARKET", product_type=product_type,
            exchange_segment="NSE_EQ", status="UNKNOWN"
        )
    ...
    try:
        ...
        if is_error_response(res) or str(res.get("status", "")).lower() in ("failure", "error"):
            ...
            logger.error(f"[ORDERS] Dhan rejected SELL: trade={trade_id} err={err_msg}")
            if ord_item:
            ...
            db_resp.commit()
            log_trade_event(db_resp, trade_id, "EXIT_ORDER_REJECTED", err_msg, quantity=qty)
            if ord_item:
            ...
            return AtsOrder(
                ...
                order_purpose=purpose_str, transaction_type="SELL", security_id=str(security_id),
                quantity=int(qty), order_type="MARKET", product_type=product_type,
                exchange_segment="NSE_EQ", status="REJECTED"
            )
        ...
        db_resp.commit()
        log_trade_event(db_resp, trade_id, "EXIT_ORDER_ACCEPTED", f"MARKET SELL accepted by Dhan: dhan_order_id={dhan_order_id}", quantity=qty)
        logger.info(f"[ORDERS] MARKET SELL accepted by Dhan: trade={trade_id}, qty={qty}, dhan_order_id={dhan_order_id}")
        ...
        return AtsOrder(
            id=order_rec_id, trade_id=trade_id, dhan_order_id=dhan_order_id,
            correlation_id=correlation_id, order_purpose=purpose_str, transaction_type="SELL",
            security_id=str(security_id), quantity=int(qty), order_type="MARKET",
            product_type=product_type, exchange_segment="NSE_EQ", status="TRANSIT"
        )
    finally:
        db_resp.close()
...
def confirm_exit_fill(
    ...
    Deducts delta_qty from trade.remaining_quantity and transitions trade state.
    """
    new_total_fill_qty = fill_qty
    if fill_price <= 0 or new_total_fill_qty <= 0:
        logger.error(f"[ORDERS] Invalid fill details for trade {trade_id}: price={fill_price}, qty={new_total_fill_qty}")
        return None
    ...
    try:
        ...
        if not trade:
            logger.error(f"[ORDERS] confirm_exit_fill: trade {trade_id} not found in DB")
            return None
        ...
        if delta_qty <= 0:
            logger.info(f"[ORDERS] Idempotent fill confirmation: trade {trade_id} already processed fill_qty={current_order_fill}. Aborting.")
            db.rollback()
            return trade
        ...
        if new_rem == 0:
            ...
            log_trade_event(db, trade_id, "EXIT_ORDER_FILLED", f"Exit order filled @ ₹{fill_price} (delta={delta_qty}). Trade CLOSED.", price=fill_price, quantity=delta_qty)
            log_trade_event(db, trade_id, "TRADE_CLOSED", f"Trade CLOSED with confirmed broker fill price=₹{fill_price}, P&L=₹{trade.realized_pnl}", price=fill_price, quantity=delta_qty)
            logger.info(f"[ORDERS] Trade {trade_id} CLOSED on broker fill confirmation: fill_price=₹{fill_price}, P&L=₹{trade.realized_pnl}")
            ...
            log_trade_event(db, trade_id, "EXIT_ORDER_PARTIALLY_FILLED", f"Exit order fill delta={delta_qty} @ ₹{fill_price}. Remaining={new_rem}.", price=fill_price, quantity=delta_qty)
            logger.info(f"[ORDERS] Trade {trade_id} PARTIAL_EXIT confirmed: delta={delta_qty} @ ₹{fill_price}, remaining={new_rem}")
        ...
        logger.error(f"[ORDERS] confirm_exit_fill failed for trade {trade_id}: {exc}", exc_info=True)
        try:
            db.rollback()
        except Exception:
            pass
...
class OrderExecutor:
    ...
    def place_entry_order(
        ...
        *,
        ...
        try:
            ...
            log_trade_event(db, trade_id, "ENTRY_ORDER_REQUESTED", f"MARKET BUY {quantity} @ MARKET for sec {security_id}", quantity=quantity)
            ...
            if is_error_response(res) or str(res.get("status", "")).lower() in ("failure", "error"):
                remarks = res.get("remarks") or res
                err_msg = str(remarks.get("error_message") if isinstance(remarks, dict) else remarks)
                logger.error(f"[ORDERS] Dhan rejected entry: trade={trade_id} err={err_msg}")
                ...
                db.commit()
                log_trade_event(db, trade_id, "ENTRY_REJECTED", err_msg, quantity=quantity)
            ...
            logger.info(f"[ORDERS] Entry MARKET BUY placed: trade={trade_id}, qty={quantity}, dhan_order_id={dhan_order_id}")
            return {"status": "placed", "trade_id": trade_id, "order_rec_id": order_rec.id, "dhan_order_id": dhan_order_id}

        except Exception as exc:
            logger.error(f"[ORDERS] Unexpected error placing entry for sec {security_id}: {exc}", exc_info=True)
            try:
                db.rollback()
            except Exception:
                pass
    ...
    def confirm_entry_fill(self, trade_id: str, fill_price: float, fill_qty: int) -> Optional[Trade]:
        ...
        try:
            ...
            if entry_order and entry_order.status == "FILLED":
                logger.debug(f"[ORDERS] Idempotent entry fill ignored: trade {trade_id} already FILLED.")
                return trade
            ...
            db.commit()
            db.refresh(trade)
            log_trade_event(db, trade_id, "ENTRY_FILLED", f"fill_price={fill_price}, fill_qty={fill_qty}.", price=fill_price, quantity=fill_qty)
            return trade
        finally:
            db.close()
    ...
    def cancel_order(self, dhan_order_id: str) -> dict:
        res = cancel_dhan_order(dhan_order_id)
