"""
trading/trade_manager.py — Central Trade Execution Engine & Tick Evaluator
==========================================================================
TradeEngine receives real-time WebSocket ticks, evaluates active trade state,
progresses trailing stop-loss stages, and triggers partial and full market exits.
"""
...
from trading.trades import get_cache_manager, TradeCacheManager

logger = logging.getLogger("ats.trading.trade_manager")
...
class TradeEngine:
    ...
    async def _apply_stage_upgrade(
        ...
        """Persist the SL stage upgrade and stop_price update immediately to PostgreSQL, then update cache."""
        tid = str(trade.id)
        entry = trade.entry_price or 0.0
        ...
        try:
            ...
            log_trade_event(
                ...
                f"Trailing SL upgraded: {stage_label} LTP={ltp}, new SL={new_sl}",
                price=ltp,
            )
        finally:
            db.close()
    ...
    async def _execute_partial_exit(self, trade: Trade, ltp: float, qty_pct: float, stage: int) -> None:
        ...
        try:
            ...
            log_trade_event(
                db, tid, f"STAGE_{stage}_PARTIAL_HIT",
    ...
    async def _record_gap(self, trade_id: str, ltp: float, gap_type: str) -> None:
        ...
        try:
            ...
            if t:
                ...
                db.commit()
            log_trade_event(db, trade_id, "GAP_DETECTED", f"type={gap_type} LTP={ltp}", price=ltp)
        finally:
            db.close()
